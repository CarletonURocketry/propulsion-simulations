...
% previous iteration. This version uses the original unedited HRAP model.
...
% output - stores past states (used for certain calcualtions)
%
% Outputs:
% state - returns the updated state of the system
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [state] = HRAP_Tank(param,state,output)
...
elseif state.m_ox_l >= state.m_l_unvaporized && state.m_ox_l > 0
    ...
    %store change in pressure from last step (K)
    state.delta_P_tank = oxidizer.P_v - state.P_tank;
    %get oxidizer porperties
    oxidizer = Nitrous_Properties_Rev1(state.T_tank);
    ...
    while delta_Z >= 0.000001
        %calcualte T2 (K)
        T_2 = T_1*(((Z_guess*m_ox_2)/(Z_old*m_ox_1))^(0.3));
        %calcualte P2 (Pa)
        P_2 = P_1*((T_2/T_1)^(1.3/0.3));
