...
if strcmp(param.model_reg,'shift_OF')
...
else %param.model_reg = const_OF
    %calcualte mass flow of fuel away from grain (kg/s)
    state.m_dot_fuel = state.m_dot_inj/param.const_OF;
    %calucalte regression rate (m/s)
    state.r_dot_grn = state.m_dot_fuel/(param.rho_fuel*pi*state.id_grn*param.l_grn);
end
