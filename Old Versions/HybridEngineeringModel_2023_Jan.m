...
for y = 0:step_stop
    ...
    if mode == 2
        ...
        if x_tank == 1
            ...
            v_2l = interpolate(P_amb,2,4,oxProps);
            v_2g = interpolate(P_amb,2,16,oxProps);
            v_2ratio = interpolate_ratio(P_amb,2,4,16,oxProps);
            h_ox = interpolate(T_tank_ev(y+1,2),1,6,oxProps);
            h_2l = interpolate(P_amb,2,6,oxProps);
            h_2g = interpolate(P_amb,2,18,oxProps);

            s_ox = interpolate(T_tank_ev(y+1,2),1,7,oxProps);
            s_2l = interpolate(P_amb,2,7,oxProps);
            s_2g = interpolate(P_amb,2,19,oxProps);
            ...
            % for some reason Moody doesn't work with very low back
            % pressure
        ...
        for z = 1:1000
            ...
            if x_tank == 1
                ...
                v_2l = interpolate(P_c,2,4,oxProps);
                v_2g = interpolate(P_c,2,16,oxProps);
                v_2ratio = interpolate_ratio(P_c,2,4,16,oxProps);
                h_ox = interpolate(T_tank_ev(y+1,2),1,6,oxProps);
                h_2l = interpolate(P_c,2,6,oxProps);
                h_2g = interpolate(P_c,2,18,oxProps);

                s_ox = interpolate(T_tank_ev(y+1,2),1,7,oxProps);
                s_2l = interpolate(P_c,2,7,oxProps);
                s_2g = interpolate(P_c,2,19,oxProps);

                x2 = (s_ox-s_2l)/(s_2g-s_2l);
                % ^ ox quality downstream of injector

                % below not used ***
                babitskiy = @(x) sqrt(2.*(h_ox-(x.*h_2g+(1-x).*h_2l)))-sqrt(2.*(x.*v_2g+(1-x).*v_2l).*(P_tank_ev(y+1,2)-P_c));
                %babitskiy = @(x) h_ox-(x.*h_2g+(1-x).*h_2l)-(x.*v_2g+(1-x).*v_2l).*(P_tank_ev(y+1,2)-P_c);
    ...
    if x_tank >= 1 || vap
        ...
        alpha_ox = (1+kappa_ox*(1-sqrt(T_tank/T_cr)))^2;
        P_2 = R_ox*T_tank/(V_tank/m_ox+b_ox)-a_ox*alpha_ox/((V_tank/m_ox)^2+2*b_ox*V_tank/m_ox-b_ox^2);

        % fudged to make tank pressure continuous. This is to match the
        % behaviour in Waterloo's static fire
        if ~vap
            P_fudge = P_tank_ev(y,2) - P_2;
        end
