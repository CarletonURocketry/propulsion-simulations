...
function [m_dot,state] = Injector_Mass_Flow(state,model,A,Cd,delta_P)

P_2 = state.P_tank - delta_P;
...
elseif model == "HEM"
    ...
    h_ox_l = CoolProp.PropsSI('H','T',state.T_tank,'Q',0,state.ox_name);
    %clauclate the tank entropy of liquid (J/kg*K)
    s_ox_l = CoolProp.PropsSI('S','T',state.T_tank,'Q',0,state.ox_name);
    ...
    if state.t == 0
        ...
        while delta_m_dot >= 0
            ...
            h_2 = CoolProp.PropsSI('H', 'P', state.P_2_crit, 'S', s_ox_l, state.ox_name);
            %clacualte the downstream denstiy (kg/m^3)
            rho_2 = CoolProp.PropsSI('D', 'P', state.P_2_crit, 'S', s_ox_l, state.ox_name);
    ...
    if  P_2 <= state.P_2_crit
        ...
        %calculate the downstream specific enthalpy (J/kg)
        h_2 = CoolProp.PropsSI('H', 'P', P_2, 'S', s_ox_l, state.ox_name);
        %clacualte the downstream denstiy (kg/m^3)
        rho_2 = CoolProp.PropsSI('D', 'P', P_2, 'S', s_ox_l, state.ox_name);
        %calculate the mass flow rate (kg/s)
        m_dot = Cd * A * rho_2 * sqrt(2 * (h_ox_l - h_2));
    end
elseif model == "Dyer"
    %calcualte m_dot_SPI (kg/s)
    [m_dot_SPI,state] = Injector_Mass_Flow_Rev_1(state,state,'SPI',A,Cd,delta_P);
    %calcualte m_dot_HEM (kg/s)
    [m_dot_HEM,state] = Injector_Mass_Flow_Rev_1(state,state,'HEM',A,Cd,delta_P);
    %calcualte non equilibrium stateeter k
...
elseif model == "PGC_v"
    %find Cp (J/kg*K)
    Cp = CoolProp.PropsSI('CP0MASS','T',state.T_tank,'D',state.rho_ox_v,state.ox_name);
    %find Cv (J/kg*K)
    Cv = CoolProp.PropsSI('CVMASS','T',state.T_tank,'D',state.rho_ox_v,state.ox_name);
