...
function [state] = Energy_Tank(state,output)
...
if state.m_ox_l > 0
    ...
    state.U_ox_total = state.U_ox_total - (state.m_dot_inj*state.dt*state.h_ox_l) - (state.m_dot_vent*state.dt*state.h_ox_v);
    ...
    while abs(delta_T_loop) >= 0.000001
        ...
        if state.T_tank >=  state.T_crit || state.T_tank <= state.T_triple
            fprintf("Temparute outside of saturation dome")
            break
        end
        %liquid oxidizer specific internal energy (J/kg)  
        u_ox_l = CoolProp.PropsSI('U','T',state.T_tank,'Q',0,state.ox_name);
        %liquid oxidizer specific internal energy (J/kg)
        u_ox_v = CoolProp.PropsSI('U','T',state.T_tank,'Q',1,state.ox_name);
        ...
        V_diff = state.V_tank - state.m_ox_total*(((1-state.quality_ox)/state.rho_ox_l) + (state.quality_ox/state.rho_ox_v));
    ...
    state.delta_P_tank = CoolProp.PropsSI('P','T',state.T_tank,'Q',state.quality_ox,state.ox_name) - state.P_tank;
    %tank pressure (Pa)
    ...
    %correct negative mass of liquid
    state.m_ox_l = 0;
    ...
    state.m_ox_total = state.m_ox_total - state.m_dot_inj*state.dt - state.m_dot_vent*state.dt;
    %calculate new total internal energy (J)
    state.U_ox_total = state.U_ox_total - (state.m_dot_inj*state.dt*state.h_ox_v) - (state.m_dot_vent*state.dt*state.h_ox_v);
    ...
    %find Cp (J/kg*K)
    Cp = CoolProp.PropsSI('CP0MASS','T',293.15,'P',101325,state.ox_name);
    %find Cv (J/kg*K)
    Cv = CoolProp.PropsSI('CVMASS','T',293.15,'P',101325,state.ox_name);
    %find specific heat ratio gamma
    gamma = Cp/Cv;

    blowdown_method = "real gas";
    ...
    if blowdown_method == "ideal gas"
        ...
        state.P_tank = P_2;
        %vapor oxidizer density (kg/m^3)
        state.rho_ox_v = state.m_ox_total / state.V_tank;
        %vapor oxidizer specific enthalpy (J/kg)
        state.h_ox_v = CoolProp.PropsSI('H','T',state.T_tank,'P',state.P_tank,state.ox_name);
    elseif blowdown_method == "real gas"
        %compressibility factor at beginng of time step
        Z_1 = CoolProp.PropsSI('Z','T',state.T_tank,'Q',1,state.ox_name);
